package com.kinestex.kinestexsdkkotlin.legacy
...
class KinesteXSDKLegacy {
    companion object {
        // ============================================================
        // LEGACY ARCHITECTURE (v1.x) - Will be deprecated
        // ============================================================

        private val logger = KinesteXLogger.instance
        /**
         * DEPRECATED: Static WebView reference can cause memory leaks
         * TODO: Remove in v2.0 - Callers should manage WebView lifecycle directly
         */
        @SuppressLint("StaticFieldLeak")
        @Deprecated(
            message = "Static WebView reference causes memory leaks. Store WebView instance in your Activity/Fragment instead.",
            level = DeprecationLevel.WARNING
        )
        private var cameraWebView: GenericWebView? = null

        private const val howToVideoLink =
            "https://cdn.kinestex.com/SDK%2Fhow-to-video%2Fhowtovideo.webm?alt=media&token=9c1254eb-0726-4eed-b16e-4e3945c98b65"

        /**
         * DEPRECATED: Static ExoPlayer reference can cause memory leaks
         * TODO: Remove in v2.0 - Callers should manage ExoPlayer lifecycle directly
         */
        @Deprecated(
            message = "Static ExoPlayer reference causes memory leaks. Manage ExoPlayer lifecycle in your Activity/Fragment instead.",
            level = DeprecationLevel.WARNING
        )
        private var videoPlayer: ExoPlayer? = null

        /**
         * Internal implementation of how-to video view
         * Shared by both v1.x and v2.0 APIs
         */
        private fun createHowToViewInternal(
            context: Context,
            onVideoEnd: (Boolean) -> Unit,
            videoURL: String? = howToVideoLink,  // Default value for videoURL
            onCloseClick: () -> Unit
        ): ViewGroup {
...
//            })
            ...
            val mediaItem = MediaItem.fromUri(videoURL ?: howToVideoLink)
        ...
        /**
         * @deprecated Use the new credential-free pattern. Initialize SDK first with `KinesteXSDK.initialize()`,
         * then call the overload without apiKey/companyName/userId parameters.
         */
        @Deprecated(
            message = "Use new credential-free API. Call KinesteXSDK.initialize() first, then use createMainView() without credentials.",
            replaceWith = ReplaceWith(
                "createMainView(context, planCategory, user, customParams, isLoading, onMessageReceived, permissionHandler)",
                "com.kinestex.kinestexsdkkotlin.KinesteXSDK"
            ),
            level = DeprecationLevel.WARNING
        )
        fun createMainView(
        ...
        fun createLeaderboardView(
            ...
            if (containsDisallowedCharacters(apiKey) || containsDisallowedCharacters(companyName) || containsDisallowedCharacters(
                ...
                val validUsername = if (!containsDisallowedCharacters(username)) {
                    username
                } else {
                    Log.e(
                        "WebViewManager",
                ...
                return GenericWebView(
                    ...
                    companyName = companyName,
                    userId = userId,
                    url = UrlHelper.leaderboardView(validUsername),
                    data = data,
                    isLoading = isLoading,
                    onMessageReceived = onMessageReceived,
                    permissionHandler = permissionHandler
